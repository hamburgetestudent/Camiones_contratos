// Interface representing the Contract data structure
interface Contrato {
    id_contrato: number;
    ...
    estado: 'Pendiente' | 'Asignado' | 'En Ruta' | 'Entregado';
    camion_asignado: string | null;
}
...
const fechaLimiteInput = document.getElementById('fecha_limite') as HTMLInputElement | null;
if (fechaLimiteInput) {
...
function actualizarEstadisticas(contratos: Contrato[]): void {
    ...
    if (statContratos) statContratos.innerText = totalContratos.toString();
    if (statToneladas) statToneladas.innerText = totalTons.toFixed(1);
    if (statTarifas) statTarifas.innerText = formatearMoneda(totalTarifas);
}

// Dynamically build and render the contract cards list
function renderizarContratos(contratos: Contrato[]): void {
    ...
    contratos.forEach(contrato => {
        ...
        if (contrato.estado === 'Asignado') claseEstado = 'asignado';
        if (contrato.estado === 'En Ruta') claseEstado = 'en-ruta';
        if (contrato.estado === 'Entregado') claseEstado = 'entregado';
...
async function crearContrato(event: Event): Promise<void> {
    event.preventDefault();

    const tipoMaterialEl = document.getElementById('tipo_material') as HTMLInputElement | null;
    const cantidadCargaEl = document.getElementById('cantidad_carga') as HTMLInputElement | null;
    const tarifaPropuestaEl = document.getElementById('tarifa_propuesta') as HTMLInputElement | null;
    const origenEl = document.getElementById('origen') as HTMLInputElement | null;
    const destinoEl = document.getElementById('destino') as HTMLInputElement | null;
    const fechaLimiteEl = document.getElementById('fecha_limite') as HTMLInputElement | null;
    ...
    try {
        ...
        if (respuesta.ok) {
            const form = document.getElementById('formulario-registro') as HTMLFormElement | null;
            if (form) form.reset();
...
async function asignarCamion(id: number): Promise<void> {
    const entradaPlaca = document.getElementById(`placa-${id}`) as HTMLInputElement | null;
    if (!entradaPlaca) return;
...
async function cambiarEstado(id: number, nuevoEstado: string): Promise<void> {
    try {
...
// Delete a contract
async function eliminarContrato(id: number): Promise<void> {
...
(window as any).crearContrato = crearContrato;
(window as any).asignarCamion = asignarCamion;
(window as any).cambiarEstado = cambiarEstado;
(window as any).eliminarContrato = eliminarContrato;
...
if (btnDocs) {
    btnDocs.addEventListener('click', () => {
        if ((window as any).electronAPI && typeof (window as any).electronAPI.openExternal === 'function') {
            (window as any).electronAPI.openExternal(`${urlBase}/docs`);
        } else {
